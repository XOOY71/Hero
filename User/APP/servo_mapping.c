...
void motor_mapping_init(void)
{
	...
	SineTarget_Init(&joint_sine, PI, 0.5f, 0.0f, 0.0f);
	for (uint8_t i = 0;i < 5; i++)
	{	
		Motor_ENABLE(&hfdcan2 , 0x01);
...
/**
  ...
  * @retval None
  */
static float ref_pos_prev = 0.0f;
static float ref_vel = 0.0f;
void Set_Taget_Torque(void)
{
#if !debug
	float dt = 0.001f;
	float target_pos = SineTarget_Update(&joint_sine, dt);

	float raw_ref_vel = (target_pos - ref_pos_prev) / dt;

	float alpha = 0.2f;
	ref_vel += alpha * (raw_ref_vel - ref_vel);

	ref_pos_prev = target_pos;

	float t_lead = 0.015f;
	float send_pos = target_pos + t_lead * ref_vel;
    ...
    VOFA_Send6(target_pos, MIT_MOTOR_MEASURE.pos, ref_vel, MIT_MOTOR_MEASURE.vel, 0, 0);
#endif
