...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    //遥控器设置模式
    if (switch_is_down(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]) && switch_is_down(chassis_move_mode->chassis_RC->rc.s[CHASSIS_FOLLOW_CHANNEL]))
    {
        //can change to CHASSIS_ZERO_FORCE,CHASSIS_NO_MOVE,CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW,
        //CHASSIS_ENGINEER_FOLLOW_CHASSIS_YAW,CHASSIS_NO_FOLLOW_YAW,CHASSIS_OPEN,CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW
        //以上参数均可选择
    ...
    else if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL]))
    {
        chassis_behaviour_mode = CHASSIS_NO_MOVE;
		 ...
		 //遥控器在下档，根据键盘cxz来改变模式，每2个毫秒刷新一次，所以要一直按着才能保证模式正确
...
static void chassis_no_follow_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
		*wz_set = -chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_WZ_CHANNEL] * CHASSIS_OPEN_RC_SCALE * 0.1 * 0.01;
}
