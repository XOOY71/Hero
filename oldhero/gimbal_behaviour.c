...
void gimbal_behavour_set(gimbal_control_t *gimbal_mode_set);
void gimbal_zero_force_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set);
void gimbal_init_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set);
void gimbal_cali_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set);
...
void gimbal_motionless_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set);
void gimbal_spin_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set);
...
void gimbal_behaviour_mode_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_MOTIONLESS)
    {
		...
		else if(gimbal_behaviour == GIMBAL_SPIN)
		{
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
		}
...
void gimbal_behaviour_control_set(fp32 *add_yaw, fp32 *add_pitch, gimbal_control_t *gimbal_control_set)
{
	...
	{
		gimbal_spin_control(add_yaw, add_pitch, gimbal_control_set);
	}
}
...
{
    if (gimbal_behaviour == GIMBAL_INIT || gimbal_behaviour == GIMBAL_CALI || gimbal_behaviour == GIMBAL_ZERO_FORCE || gimbal_behaviour == GIMBAL_MOTIONLESS)
    {
        return 1;
    }
    else
...
void gimbal_behavour_set(gimbal_control_t *gimbal_mode_set)
{
	static bool can_change_to_GIMBAL_RELATIVE_ANGLE = false;
	
	if(switch_is_mid(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]))
	{
		can_change_to_GIMBAL_RELATIVE_ANGLE = true;
	}
	...
	if (gimbal_behaviour == GIMBAL_INIT)
	{
		...
		if (init_time < GIMBAL_INIT_TIME && init_stop_time < GIMBAL_INIT_STOP_TIME &&
				!switch_is_mid(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]) && !toe_is_error(DBUS_TOE))
		{
			return;
		}
		else
	...
	else if (switch_is_down(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]) && can_change_to_GIMBAL_RELATIVE_ANGLE)
	{
		gimbal_behaviour = GIMBAL_RELATIVE_ANGLE;
	}

	//遥控器右上拨杆在下档，根据键盘shift、x、c来改变模式
	if (switch_is_down(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]) &&  (gimbal_mode_set->gimbal_rc_ctrl->key.v & GIMBAL_ZERO_KEYBOARD))
	{
		gimbal_behaviour = GIMBAL_MOTIONLESS;		//x键 无力
	}
	else if(switch_is_down(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]) && (gimbal_mode_set->gimbal_rc_ctrl->key.v & GIMBAL_SPIN_KEYBOARD))
	{
		gimbal_behaviour = GIMBAL_SPIN;		//shift键 小陀螺
	}
	else if(switch_is_down(gimbal_mode_set->gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]) && (gimbal_mode_set->gimbal_rc_ctrl->key.v & GIMBAL_RELATIVE_KEYBOARD) && can_change_to_GIMBAL_RELATIVE_ANGLE)
	{
		gimbal_behaviour = GIMBAL_RELATIVE_ANGLE;		//c键 跟随
	}
...
/**
  * @brief          云台进入小陀螺模式，pitch电机与底盘独立,yaw轴电机跟随底盘转动
  * @author         printk
  * @param[in]      yaw: yaw轴角度控制，为角度的增量 单位 rad
  * @param[in]      pitch: pitch轴角度控制，为角度的增量 单位 rad
  * @param[in]      gimbal_control_set:云台数据指针
